...
namespace Bkl.Infrastructure
{
    public class UniviewHelper
    {
        ...
        public void SetDeviceLogin(string username, string password)
        {
            _username = username;
            _password = password;
            _client = new HttpClient(new HttpClientHandler
        ...
        public void SetDeviceConnection(string host, int port)
        {
        ...
        public class SnapException : Exception
        {
            ...
            public SnapException(string host, HttpResponseMessage resp)
            {
                _resp = resp;
            }

            public HttpResponseMessage Resp { get => _resp; set => _resp = value; }
        }
        ...
        public async Task<byte[]> Snapshop(int channel, int stream)
        {
            ...
            try
            {
                resp = await _client.GetAsync($"http://{_host}:{_port}/LAPI/V1.0/Channels/{channel}/Media/Video/Streams/{stream}/Snapshot");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"SnapshopError {_host} {_port} {_username} {_password} ");
                Console.WriteLine($"SnapshopError {ex.Message} ");
                return null;
            }
            if (resp.StatusCode != HttpStatusCode.OK)
            {
                Console.WriteLine($"{_host} {_port} {_username} {_password} error " + resp.Content.ReadAsStringAsync().Result);
                throw new SnapException(_host, resp);
            }
            try
            {
            ...
            {
                Console.WriteLine($"SnapshopErrorGetData {ex.Message} ");
                return null;
            }
        ...
        public async Task<UniviewResponse<TemperatureValueList>> GetTemperatureValues()
        {
            try
            {
                ...
                {
                    Console.WriteLine($"JsonError {_host} {_port} data:{imgpic}");
